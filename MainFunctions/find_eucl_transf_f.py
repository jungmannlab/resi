...
def find_eucl_transf_f(folder_path, ch1_files, ch3_files):
    ...
    for i, (file1, file3) in enumerate(zip(ch1_files, ch3_files)):
        ...
        for j in range(number_of_R4_sites): # loop over the localization of each site
            ...
            #print('locsy_ch3 a', type(locsy_ch3), len(locsy_ch3))
            if flag_3D:
        ...
        #print('locsy_ch3 b', type(locsy_ch3), len(locsy_ch3))
        if flag_3D:
    ...
    if not flag_3D:
        ...
        """
        df = pd.DataFrame()
        df['pick'] = [row[0] for row in eucl_tr_params]

        R_matrices = [row[1].flatten() for row in eucl_tr_params] # from [[R11, R12, R13] [R21, R22, R23] [R31, R32, R33]] to [R11, R12, R13, R21, R22, R23, R31, R32, R33]
        print(eucl_tr_params[0][1])
        print(R_matrices[0])
        """
