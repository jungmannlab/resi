...
# =============================================================================
# Load (and optionally plot) simulated data files
# =============================================================================
 
nfiles = 100
D_opt_array = np.zeros(nfiles)
σ_opt_array = np.zeros(nfiles)
density_d_opt_array = np.zeros(nfiles)
...
for a in range(nfiles):
    ...
    distances = np.load(filename) # get the first neighbour distances
    
    binsize = 4 # nm
    bins = np.arange(0, 1000, binsize)
    freq_exp, bins = np.histogram(distances, bins=bins, density=True)
    ...
    for i, D in enumerate(D_array):
        for j, sigma_label in enumerate(σ_label_array):
            for k, density in enumerate(density_d_array):
                ...
                sq_distance = np.sum((freq_exp - freq_sim_1nn)**2) # in nm^2
